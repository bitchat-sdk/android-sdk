    ...
    fun sendPrivateFile(
        ...
        transportScope.launch {
            try {
                ...
                if (recipientNostrPubkey == null) {
                    Log.w(TAG, "No Nostr public key found for file transfer to peerID: $to")
                    return@launch
                }

                val senderIdentity = NostrIdentityBridge.getCurrentNostrIdentity(context)
                if (senderIdentity == null) {
                    Log.e(TAG, "No Nostr identity available for file transfer")
                    return@launch
                }
                ...
                val recipientHex = try {
                    ...
                    Log.e(TAG, "NostrTransport: failed to decode npub for file transfer: $e")
                    return@launch
                }

                val embedded = NostrEmbeddedBitChat.encodeFileTransferForNostr(
                    filePacket = filePacket,
                    recipientPeerID = to,
                    senderPeerID = senderPeerID
                )
                if (embedded == null) {
                    Log.e(TAG, "NostrTransport: failed to embed file transfer")
                    return@launch
                }
                ...
                Log.e(TAG, "Failed to send private file via Nostr: ${e.message}")
            }
        }
    }
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            if (peerID.length == 16) {
                ...
                if (fallbackStatus?.peerNostrPublicKey != null) return fallbackStatus.peerNostrPublicKey
            }

            com.bitchat.android.ui.groups.GroupChatManager.shared
                ?.nostrPublicKeyForPeer(peerID)
                ?.let { return it }
